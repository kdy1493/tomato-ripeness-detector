...
CONFIG = {
    ...
    # ROI (src/tracker.py 와 동일: 프레임 중앙 기준 가로 폭 2*roi_half_width)
    # None 또는 0 이면 전체 프레임
    "roi_half_width": 320,
...
def make_tracker(config: dict, fps: float) -> sv.ByteTrack:
    return sv.ByteTrack(
...
def run(config: dict) -> dict:
    ...
    trackers:      Dict[int, sv.ByteTrack]     = {cid: make_tracker(config, fps) for cid in CLASS_NAMES}
        ...
        for cid, tracker in trackers.items():
            ...
            if cls_dets.tracker_id is not None and len(cls_dets) > 0:
                ...
                new_idx.sort(key=lambda i: (
                    0.5 * (cls_dets.xyxy[i][1] + cls_dets.xyxy[i][3])   # cy
                    - 0.5 * (cls_dets.xyxy[i][0] + cls_dets.xyxy[i][2])  # cx
                ))
                for i in new_idx:
                    raw_to_stable[cid][int(cls_dets.tracker_id[i])] = next_sid[cid]
                    next_sid[cid] += 1
                cls_dets.tracker_id = np.array(
                    [raw_to_stable[cid][int(r)] for r in cls_dets.tracker_id], dtype=np.int64
                )
                seen_ids[cid].update(cls_dets.tracker_id.tolist())
